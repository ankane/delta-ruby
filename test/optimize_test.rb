...
class OptimizeTest < Minitest::Test
  def test_compact
    ...
    with_table(df) do |dt|
      ...
      assert_in_delta 517, metrics["filesAdded"]["avg"]
      assert_equal 517, metrics["filesAdded"]["max"]
      assert_equal 517, metrics["filesAdded"]["min"]
      assert_equal 1, metrics["filesAdded"]["totalFiles"]
      assert_equal 517, metrics["filesAdded"]["totalSize"]
      assert_in_delta 502, metrics["filesRemoved"]["avg"]
      assert_equal 502, metrics["filesRemoved"]["max"]
      assert_equal 502, metrics["filesRemoved"]["min"]
      assert_equal 3, metrics["filesRemoved"]["totalFiles"]
      assert_equal 1506, metrics["filesRemoved"]["totalSize"]
  ...
  def test_z_order
    ...
    with_table(df) do |dt|
      ...
      assert_equal 3, metrics["totalConsideredFiles"]
      assert_equal 0, metrics["totalFilesSkipped"]
      assert_equal false, metrics["preserveInsertionOrder"]
    end
  end
end
