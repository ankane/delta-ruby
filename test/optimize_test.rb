...
class OptimizeTest < Minitest::Test
  def test_compact
    ...
    with_table(df) do |dt|
      ...
      assert_in_delta 516, metrics["filesAdded"]["avg"]
      assert_equal 516, metrics["filesAdded"]["max"]
      assert_equal 516, metrics["filesAdded"]["min"]
      assert_equal 1, metrics["filesAdded"]["totalFiles"]
      assert_equal 516, metrics["filesAdded"]["totalSize"]
      assert_in_delta 501, metrics["filesRemoved"]["avg"]
      assert_equal 501, metrics["filesRemoved"]["max"]
      assert_equal 501, metrics["filesRemoved"]["min"]
      assert_equal 3, metrics["filesRemoved"]["totalFiles"]
      assert_equal 1503, metrics["filesRemoved"]["totalSize"]
